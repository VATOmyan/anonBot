...
class Database:
    ...
    def check(self, chat_id):
        with self.connection:
            chat2 = self.cursor.execute('SELECT `chat_two` FROM `chats` WHERE `id` = ?', (chat_id,))
            chat1 = self.cursor.execute('SELECT `chat_one` FROM `chats` WHERE `id` = ?', (chat_id,))
            print(str(chat1), str(chat2), 'chats1')
            if str(chat1) != str(chat2):
                print(str(chat1), str(chat2), 'chats2')
                return True
            else:
                return False
