...
def bot_message(message):
    if message.chat.type == 'private':
        ...
        if message.text == '🤼 Поиск собеседника':
            ...
            if dataBase.create_chat(message.chat.id, chat_two) == False and dataBase.check(message.chat.id) == True:
                print('faf')
            ...
            #     dataBase.sendMsg(message)
            # if dataBase.check(message.chat.id):
            # #print(dataBase.get_work_chat(message.chat.id))
            #     print('bbb')
            #
            # else:
            #     print(dataBase.get_work_chat(message.chat.id)[0], 'alkdadadasdadadsa')
...
def sendPhoto(message):
    bot.send_media_group(dataBase.get_work_chat(message.chat.id)[1], message.photo[0].file_id)
    print(message.photo[0].file_id)
    dataBase.sendPh(message)
